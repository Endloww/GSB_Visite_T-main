...
exports.getOneVisiteur = expressAsyncHandler(async (req, res) => {
  const visiteur = await Visiteur.findOne({ _id: req.params.id }).populate('portefeuille') ;
  if (!visiteur) {
...
exports.addPraticien = expressAsyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const estDejaPresent = visiteur.portefeuille.includes(praticien_id);
    if (estDejaPresent) {
      res.status(400).json({ message: 'Ce praticien est déjà dans le portefeuille du visiteur' });
      return;
    }

    await Visiteur.updateOne({ _id: visiteur_id }, { $push: { portefeuille: praticien_id } });
